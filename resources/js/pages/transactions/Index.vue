...
import { mapsUrl } from '@/composables/useGeolocation';
...
<template>
    ...
    <div class="space-y-4 p-4 md:p-6">
        ...
        <div class="divide-y rounded-md border text-sm md:hidden">
            <template v-for="(item, idx) in transactions.items" :key="item.id">
                <!-- Period header -->
                <div
                    v-if="isNewPeriod(idx)"
                    class="flex items-center justify-between gap-2 bg-muted/70 px-3 py-1.5"
                >
                    <span
                    ...
                    <span
                        class="flex items-center gap-2 text-xs font-semibold tabular-nums"
                    >
                        <span class="text-green-600 dark:text-green-400">
                            +{{
                                formatAmount(
                                    totals[periodKey(item.transacted_at)]
                                        ?.income ?? 0,
                                )
                            }}
                        </span>
                        ...
                        <div class="flex shrink-0 items-center gap-1">
                            ...
                            <div class="flex flex-col gap-0.5">
                                <a
                                    ...
                                    class="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground hover:text-foreground"
                                >
